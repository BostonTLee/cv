...
#section("Leadership", {
  for role in cv.leadership {
    entry-heading(
      [#role.title #linebreak() #role.entity],
      right-content: [#date-range(role.start_date, end: role.at("end_date", default: none)) #linebreak() #role.location],
    )
    v(0.25em)
    bullet-list(
      role.description,
...
#section("Awards", {
  for award in cv.awards {
    entry-heading(
      [#award.title #linebreak() #award.awarder],
      right-content: [#award.dates.map(date-label).join(", ")],
    )
    bullet-list((award.summary,))
    v(0.5em)
  }
})
